...
def draw_dets(img, dets, color=(0,255,0), thickness=2, put_fps=None, draw_boxes=True):
    ...
    if put_fps:
        cv2.putText(img, put_fps, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (40, 220, 40), 2, cv2.LINE_AA)
    return img
...
def expand_clip(x1,y1,x2,y2, margin, W,H):
    ...
    w *= (1.0 + margin)
    h *= (1.0 + margin)
    x1n = max(0, int(cx - w*0.5)); y1n = max(0, int(cy - h*0.5))
    x2n = min(W-1, int(cx + w*0.5)); y2n = min(H-1, int(cy + h*0.5))
...
class TRTInfer:
    def __init__(self, engine_path, input_name="input.1", size=640, verbose=False):
        ...
        import pycuda.driver as cuda
        cuda.init()
        import pycuda.autoinit       # creates & pushes a context for this thread
        ...
        self.h_in = cuda.pagelocked_empty((1,3,self.size,self.size), dtype=self.dtypes[self.in_name])
        self.out_names = [n for n in self.names if self.modes[n] == trt.TensorIOMode.OUTPUT]
        self.h_out = {n: cuda.pagelocked_empty(int(np.prod(self.shapes[n])), dtype=self.dtypes[n])
    ...
    def infer(self, frame_bgr):
        ...
        for n in self.out_names:
            if n not in self.alloc or n not in self.h_out: continue
    ...
    def _postproc_scrfd(self, outs, left, top, r, W, H):
        scores_map = {arr.shape[0]: arr.reshape(-1)
                      for arr in outs.values() if arr.ndim==2 and arr.shape[-1]==1}
        bboxes_map = {arr.shape[0]: arr.reshape(-1,4)
                      for arr in outs.values() if arr.ndim==2 and arr.shape[-1]==4}
        ...
        for stride in STRIDES:
            ...
            sc1 = sc1[keep]; boxes_xyxy = boxes_xyxy[keep]

            topk = int(getattr(self, "topk_per_level", TOPK_PER_LEVEL))
            if sc1.size > topk:
                idk = np.argpartition(sc1, -topk)[-topk:]
                sc1 = sc1[idk]; boxes_xyxy = boxes_xyxy[idk]

            all_scores.append(sc1); all_boxes.append(boxes_xyxy)
        ...
        if scores.size > gtopk:
            ...
            scores = scores[idg]; boxes = boxes[idg]

        dets = []
        for b, s in zip(boxes, scores):
            x1 = (b[0] - left)/r; y1 = (b[1] - top)/r
            x2 = (b[2] - left)/r; y2 = (b[3] - top)/r
            x1 = int(max(0, min(W-1, x1))); y1 = int(max(0, min(H-1, y1)))
            x2 = int(max(0, min(W-1, x2))); y2 = int(max(0, min(H-1, y2)))
            if min(x2-x1, y2-y1) < IGNORE_SHORT: continue
...
def main():
    ...
    ap.add_argument("--pixel_margin", type=float, default=0.25, help="bbox expand ratio before pixelation")
    ap.add_argument("--pixel_max_faces", type=int, default=32, help="max faces pixelated per frame")
    ap.add_argument("--pixel_noise", type=float, default=0.0, help="add noise after pixelation (0=off)")
    ...
    if args.conf is not None: infer.conf_thres = args.conf
    if args.topk is not None: infer.topk_per_level = args.topk
    if args.max_dets is not None: infer.max_dets = args.max_dets
